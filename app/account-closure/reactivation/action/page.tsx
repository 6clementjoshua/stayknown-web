// app/account-closure/reactivation/action/page.tsx
// StayKnown Secure Account Closure & Recovery
//
// Protected owner reactivation decision page.
//
// SECURITY RULES
// - GET/page load never consumes or mutates the token.
// - The raw token is never rendered as visible text.
// - A deliberate POST to /api/account-closure/reactivation/action is required.
// - The owner must acknowledge the recovery consequences and type RESTORE.
// - The backend API and atomic SQL RPC remain authoritative for token hash,
//   purpose, expiry, one-use enforcement, owner identity, request state,
//   Protection Lock state, recovery deadline and idempotency.
// - Email scanners and link-preview bots may open this page safely.
// - This page never signs the owner in and never restores the account itself.
...
export const metadata: Metadata = {
  title: "Restore StayKnown Account | StayKnown",
  description:
    "Securely review and confirm restoration of your original StayKnown account.",
  robots: {
    index: false,
    follow: false,
    noarchive: true,
  ...
  alternates: {
    canonical: "/account-closure/reactivation/action",
  },
  openGraph: {
    title: "StayKnown Protected Account Recovery",
    description: "A protected StayKnown account-reactivation decision page.",
    type: "website",
    url: "/account-closure/reactivation/action",
    siteName: "StayKnown",
  },
};
...
function ChatRestoreIcon({ className = "" }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 24 24"
      fill="none"
    ...
    >
      <path
        d="M5.2 5.3h13.6c1 0 1.7.8 1.7 1.7v8.3c0 1-.8 1.7-1.7 1.7H10l-4.7 3.1V17H5.2c-1 0-1.7-.8-1.7-1.7V7c0-1 .8-1.7 1.7-1.7Z"
        stroke="currentColor"
        strokeWidth="1.55"
        strokeLinejoin="round"
      />
...
function ReactivationForm({ token }: { token: string }) {
  return (
    <form
      method="post"
      action="/api/account-closure/reactivation/action"
      className="mt-7"
    >
      ...
      <input
        type="hidden"
        name="source"
        value="account_closure_reactivation_email_action_page"
      />

      <div className="space-y-3">
        <ConfirmationCheckbox name="restore_account_acknowledged">
          I understand that this requests restoration of my{" "}
          <strong className="text-white/88">
            original reserved StayKnown account
          </strong>
          , not creation of a new account or transfer of the identity to another
          person.
        </ConfirmationCheckbox>

        <ConfirmationCheckbox name="safety_state_acknowledged">
          I understand that restoration does not erase safety evidence and does
          not bypass an active Protection Lock. StayKnown may still require
          owner reauthentication before sensitive controls become available.
        </ConfirmationCheckbox>
      </div>
      ...
      <label className="mt-4 block">
        ...
        >
          ...
          token, the account owner, the recovery deadline, and current security
          state.
        </span>
      </label>
      ...
      >
        ...
        <RestoreIcon className="mr-3 h-5 w-5" />

        <span>Restore my StayKnown account</span>
      ...
      <p className="mt-3 text-center text-[10px] font-semibold leading-[1.55] text-white/36">
        Nothing is restored until the protected backend verifies and consumes
        this one-use token in one atomic transaction.
      </p>
    </form>
  );
}
      ...
      <div className="mx-auto w-full max-w-2xl">
        <PremiumCard className="text-center">
          ...
          <p className="mx-auto mt-5 max-w-xl text-[13px] font-semibold leading-[1.75] text-white/58 sm:text-[14px]">
            ...
            valid. Opening this page did not restore an account, release an
            identity reservation, change chat availability, or make another
            account change.
          </p>
...
export default async function AccountClosureReactivationActionPage({
  searchParams,
}: PageProps) {
      ...
      <div className="mx-auto grid w-full max-w-5xl gap-5 lg:grid-cols-[minmax(0,1.08fr)_minmax(300px,0.92fr)] lg:items-start">
        <PremiumCard className="overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <StatusPill>Account not restored</StatusPill>
          ...
          <div className="mt-7 grid gap-6 sm:grid-cols-[auto_1fr] sm:items-start">
            ...
            <div className="min-w-0">
              ...
              <h1 className="mt-3 text-[36px] font-black leading-[0.96] tracking-[-0.065em] text-white sm:text-[46px]">
                Restore your original StayKnown account?
              </h1>

              <p className="mt-5 text-[13px] font-semibold leading-[1.72] text-white/60 sm:text-[14px]">
                This action asks StayKnown to end the reversible closure period
                and return the same reserved identity. It does not create a new
                account and does not transfer your email address, username,
                chats, contacts, or safety history to another person.
              </p>
            </div>
          </div>
          ...
          <div className="mt-7 rounded-[22px] border border-[#92f3cf]/[0.14] bg-[#92f3cf]/[0.045] p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.07)]">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1.5 text-[11px] font-semibold leading-[1.62] text-white/52">
                  StayKnown does not consume reactivation tokens on page load.
                  Email scanners, link previews, accidental taps, and forwarded
                  previews cannot restore the account.
                </p>
              </div>
            </div>
          </div>

          <ReactivationForm token={token} />
        </PremiumCard>
        ...
        <div className="space-y-5">
          <PremiumCard>
            ...
            <div className="mt-4 space-y-3">
              <RecoveryPoint
                ...
                title="Same reserved identity"
                body="StayKnown restores the original account tied to its protected email and username reservation. The recovery action does not create a duplicate identity."
              />

              <RecoveryPoint
                icon={<ChatRestoreIcon className="h-5 w-5" />}
                title="Chats and relationships reconcile"
                body="Read-only chat impacts and closure contact notices are restored through protected backend reconciliation. Existing history and safety evidence are preserved."
              />

              <RecoveryPoint
                icon={<ShieldIcon className="h-5 w-5" />}
                title="Security remains authoritative"
                body="An unresolved Protection Lock, expired recovery deadline, legal hold, invalid token, or identity mismatch can still block restoration."
              />
            </div>
          </PremiumCard>
          ...
          <PremiumCard className="border-white/[0.075]">
            <h2 className="text-[15px] font-black tracking-[-0.025em] text-white">
              What restoration does not erase
            </h2>

            <ul className="mt-4 space-y-3">
              {[
                "Required security and account-closure audit evidence.",
                "Eligible safety records preserved under StayKnown policy.",
                "Evidence needed for abuse prevention, consent, billing, legal obligations, or dispute handling.",
                "A separate active Protection Lock that still requires verified owner reauthentication.",
              ].map((item) => (
                <li
                  key={item}
            ...
            <div className="mt-5 rounded-[18px] border border-white/[0.10] bg-white/[0.035] p-4">
              ...
              <p className="mt-2 text-[11px] font-semibold leading-[1.6] text-white/48">
                A trusted contact or recovery contact may assist with safety,
                but cannot use this page to sign in, restore, unlock, close, or
                control another person’s StayKnown account.
              </p>
            </div>
        ...
        <footer className="lg:col-span-2">
          <div className="border-t border-white/[0.075] pt-7 text-center">
            ...
            <p className="mt-5 text-[10px] font-semibold leading-[1.6] text-white/34">
              Account restoration does not replace police, ambulance, fire
