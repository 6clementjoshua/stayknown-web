  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey) {
      return NextResponse.json(
        {
          ok: false,
          error: "Missing SUPABASE_URL or SUPABASE_ANON_KEY in Vercel.",
        },
        { status: 500 },
      );
    }
      ...
      return NextResponse.json(
        {
          ok: false,
          error:
            "Missing SUPABASE_SERVICE_ROLE_KEY in Vercel server environment.",
        },
        { status: 500 },
      );
    }
    ...
    // Service-role client is server-only. It bypasses RLS so we can verify
    // whether this email is allowed before sending the magic login link.
    ...
    // Anon client is used for normal passwordless auth.
